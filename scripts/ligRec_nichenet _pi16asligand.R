...
setwd(SP)
resu = "LigRecPairs/"
markers.fi = "results_DeOpNEWseu/FI-DeOp_ALLMARKERS_Pos_integratedD0.txt"
refclunum = 0 # faps smoc2 chosen as "reference" , can vary depending on bio question
# here the question is to check pi16, so smoc2 cells can be taken as preceding stage
outfile = "ligrec_pi16aslig_heatmap-FI.pdf"
...
seu =  readRDS(paste0("rds_NEWseu/FI-DeOp_integrated_seu_fitsne.rds"))
celltv = readRDS(paste0("rds_NEWseu/FI-DeOp_integr_celltype_Vector.rds"))
seuratObj = RenameIdents(seu,celltv)
...
#  note: "?FAPs??"  not detected in this new FILTERED seurat object version
receivers <- c("FAPS, Smoc2+","Endothelial cells","unknown",
                          "Endothelial_lymphatic","FAPs, Smoc2+","Myonuclei","B_lymphocytes.CD4",
                          "Smooth muscle","MuSc","Tenocytes","Early T","FAPs?, Smoc2++",
                          "Schwann cells","Neutrophyls_macrophages","Schwann cells")  
expressed_genes_receiver = get_expressed_genes(receivers[1], seuratObj, pct = 0.1) # to initialize with first cluster,
for (k in 2:length(receivers)){     # then add all the rest
  tmpvec = get_expressed_genes(receivers[k], seuratObj, pct = 0.1)
...
list_expressed_genes_sender = sender_celltypes %>% unique() %>% lapply(get_expressed_genes, seuratObj, 0.1) # lapply to get the expressed genes of every sender cell type separately here
expressed_genes_sender = list_expressed_genes_sender %>% unlist() %>% unique()


# receiver : define differentially expr in two conditions, 
# but here I have only one : day zero !
# seurat_obj_receiver= subset(seuratObj, idents = receivers)
...
savedmarkers <- read.table(markers.fi, sep="\t", header =T)

geneset_oi1 <- savedmarkers$gene[savedmarkers$cluster==refclunum & savedmarkers$avg_logFC > 1]
...
pdf(paste0(resu,outfile), width=15)
